...
impl Parser {
    ...
    pub fn is_generic(&self, mut ahead:usize) -> Result<(bool, usize)> {
        ...
        let Token{kind:TokenKind::Identifier(_), .. } = self.peek_at(ahead)? else {
            return Ok((false, ahead));
        };
        
        if let TokenKind::Lt = self.peek_at(ahead+1)?.kind && let (flag, n) = self.is_generic(ahead+2)? {
            if !flag {
                return Ok((false, ahead - initial_ahead));
            }
            ahead += n + 2;
        }
        return Ok((matches!(self.peek_at(ahead+1)?.kind, TokenKind::Gt), ahead - initial_ahead))
    }
