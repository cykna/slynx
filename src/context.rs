...
impl SlynxContext {
    ...
    pub fn start_compilation<S: SlynxCompiler>(self, compiler: S) -> Result<()> {
        ...
        let stream = Lexer::tokenize(self.get_entry_point_source())
            .map_err(|e| match e {
            ...
            .wrap_err("Lexing failed")?;
        let decls = Parser::new(stream)
            .parse_declarations()
            .map_err(|e| match e {
        ...
        hir.generate(decls)
            ...
            .wrap_err("HIR generation failed")?;
