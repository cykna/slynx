...
impl SlynxHir {
        ...
        match HirType::new(name) {
            ...
            Err(_) => {
                if let Some(name_id) = self.symbols_module.retrieve(&name.identifier)
                    && let Some(ty) = self.types_module.retrieve_id(name_id)
                {
    ...
    pub fn retrieve_information_of(&mut self, name: &str, span: &Span) -> Result<(TypeId, &HirType)> {
        if let Some(name_id) = self.symbols_module.retrieve(name)
            && let Some(id) = self.types_module.get_id(name_id) && let Some(ty) = self.types_module.get_type(id)
        {
            Ok((*id, &ty))
        } else {
            Err(HIRError {
