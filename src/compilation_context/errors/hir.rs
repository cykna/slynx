    ...
    fn hir_error_to_string(&self, hir: &SlynxHir, err: &HIRError) -> String {
        let out = match &err.kind {
            HIRErrorKind::InvalidFuncallArgLength {
                ...
                let func_name = hir.symbols_module.get_name(*func_name);
                format!(
            ...
            HIRErrorKind::NotAFunction(name, ty) => {
                let name = hir.symbols_module.get_name(*name);
                format!(
            ...
            HIRErrorKind::PropertyNotVisible { prop_name } => {
                let prop_name = hir.symbols_module.get_name(*prop_name);
            ...
            HIRErrorKind::InvalidType { ty, reason } => {
                let ty = hir.symbols_module.get_name(*ty);
            ...
            HIRErrorKind::MissingProperty { prop_names } => {
                let names = prop_names
                    .iter()
                    .map(|v| hir.symbols_module.get_name(*v))
                    .collect::<Vec<&str>>()
                    .join(", ");
