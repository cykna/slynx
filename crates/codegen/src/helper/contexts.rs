...
use crate::{Component, Function, IRPointer, IRType, IRTypeId, Instruction, Label, SlynxIR, Value};

impl SlynxIR {
    #[inline]
    ///Retrieves the context from its provided `func`
    pub(crate) fn get_function(&self, func: IRPointer<Function, 1>) -> &Function {
        &self.functions[func.ptr()]
    }
    #[inline]
    ///Retrieves the context from its provided `func`
    pub(crate) fn get_function_mut(&mut self, func: IRPointer<Function, 1>) -> &mut Function {
        &mut self.functions[func.ptr()]
    }
    ...
    pub fn return_type_of_context(&self, ir: IRPointer<Function, 1>) -> IRTypeId {
        let ctx = self.get_function(ir);
        let id = ctx.ty();
    ...
    ///Returns the return type of the given context `ir`.
    pub fn arg_types_of_context(&self, ir: IRPointer<Function, 1>) -> &[IRTypeId] {
        let ctx = self.get_function(ir);
        let id = ctx.ty();
        match self.types.get_type(id) {
            IRType::Function(func) => {
    ...
    pub fn get_labels_of(&self, ir: IRPointer<Function, 1>) -> &[Label] {
        let ptr = self.functions[ir.ptr()].labels_ptr();
        &self.labels[ptr.range()]
    }

    ///Gets the labels of the given context `ir`.
    pub fn get_labels_mut_of(&mut self, ir: IRPointer<Function, 1>) -> &mut [Label] {
        let ptr = self.functions[ir.ptr()].labels_ptr();
    ...
    pub fn insert_label(&mut self, ir: IRPointer<Function, 1>, label: &str) -> IRPointer<Label, 1> {
