...
use crate::{
    Component, Function, IRComponent, IRError, IRPointer, IRType, IRTypeId, Instruction, Operand,
    SlynxIR, TempIRData,
};
...
impl SlynxIR {
    ...
    pub fn create_blank_function(&mut self, name: SymbolPointer) -> IRPointer<Function, 1> {
        let context = Function::new(name, self.types.create_empty_function());
